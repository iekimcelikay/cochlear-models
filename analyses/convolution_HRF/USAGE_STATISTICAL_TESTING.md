...
## Important: BEZ Files are Separated by Fiber Type

Your BEZ convolved responses are stored in separate files by fiber type in `results/bez/`:
- `BEZ_convolved_responses_hsr_20251027_155115.pkl` - High spontaneous rate fibers
- `BEZ_convolved_responses_msr_20251027_155115.pkl` - Medium spontaneous rate fibers  
- `BEZ_convolved_responses_lsr_20251027_155115.pkl` - Low spontaneous rate fibers

**Important**: You must use the correct file that matches the `--fiber_type` you want to analyze:
- For `--fiber_type hsr` → use `BEZ_convolved_responses_hsr_20251027_155115.pkl`
- For `--fiber_type msr` → use `BEZ_convolved_responses_msr_20251027_155115.pkl`
- For `--fiber_type lsr` → use `BEZ_convolved_responses_lsr_20251027_155115.pkl`
...
python /home/ekim/PycharmProjects/subcorticalSTRF/subcorticalSTRF/convolution_HRF/statistical_testing_bez_runs.py \
  --bez_file results/bez/BEZ_convolved_responses_hsr_20251027_155115.pkl \
...
```bash
python /home/ekim/PycharmProjects/subcorticalSTRF/subcorticalSTRF/convolution_HRF/statistical_testing_bez_runs.py \
  --bez_file results/bez/BEZ_convolved_responses_msr_20251027_155115.pkl \
  --analyze_single_condition \
  --fiber_type msr \
  --cf_val 2000 \
  --freq_val 2000 \
...
```bash
python /home/ekim/PycharmProjects/subcorticalSTRF/subcorticalSTRF/convolution_HRF/statistical_testing_bez_runs.py \
  --bez_file results/bez/BEZ_convolved_responses_lsr_20251027_155115.pkl \
  --analyze_single_condition \
  --fiber_type lsr \
  --cf_val 500 \
  --freq_val 500 \
...
### Getting Help
```bash
python /home/ekim/PycharmProjects/subcorticalSTRF/subcorticalSTRF/convolution_HRF/statistical_testing_bez_runs.py --help
```
