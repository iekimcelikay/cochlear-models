%% Plot BEZ2018 mean firing rates
% Loads precomputed mean rates from LATEST_extract_psth_1709_for128fiberpsth.m
% Uses same plotting logic as compute_cochlea_mean_rates.m

clear; clc;

% Path to the saved mean rates file from BEZ script
input_dir = "/home/ekim/PycharmProjects/subcorticalSTRF/subcorticalSTRF/BEZ2018_meanrate/results/processed_data";
mean_rates_file = fullfile(input_dir, 'mean_rates_128fibers.mat');

% Load precomputed mean_rates
assert(isfile(mean_rates_file), 'Mean rates file not found: %s', mean_rates_file);
S = load(mean_rates_file, 'mean_rates', 'sem_rates');
mean_rates = S.mean_rates;
sem_rates = S.sem_rates;

% Load the PSTH data file to get metadata
psth_data_file = fullfile(input_dir, 'psth_data_128fibers.mat');
assert(isfile(psth_data_file), 'PSTH data file not found: %s', psth_data_file);
P = load(psth_data_file, 'cfs', 'frequencies', 'dbs');

% Extract metadata
unique_cf = cellfun(@str2double, P.cfs);
unique_tone = cellfun(@str2double, P.frequencies);
unique_db = P.dbs;
...
for icf = 1:nCF
    ...
    for fti = 1:numel(fiber_types)
        ...
        for idb = 1:nDB
            ...
            errorbar(unique_tone, y, err, 'Color', color, 'LineStyle', linestyle, ...
...
sgtitle('Mean Firing Rate vs Tone Frequency (BEZ2018 Model, n=10 runs)', 'FontSize', 14);

% Save the overview plot with higher resolution
saveas(gcf, fullfile(input_dir, 'bez_tuning_overview.png'));
print(fullfile(input_dir, 'bez_tuning_overview_highres.png'), '-dpng', '-r300');
...
for i = 1:length(cf_targets)
    ...
    title(sprintf('CF %.0f Hz (BEZ2018 Model, n=10 runs)', unique_cf(idxCF)), 'FontSize', 12);
    ...
    saveas(gcf, fullfile(input_dir, sprintf('bez_tuning_CF%.0fHz.png', unique_cf(idxCF))));
...
db_idx = 3; % Choose a specific dB level for comparison
...
for i = 1:length(cf_targets)
    ...
    for fti = 1:numel(fiber_types)
        ...
        err = squeeze(sem_rates.(ft)(idxCF, :, db_idx));
        
        errorbar(unique_tone, y, err, 'Color', cmap(fti,:), 'LineWidth', 2, ...
...
sgtitle(sprintf('BEZ2018 Tuning Curves at %d dB', unique_db(db_idx)), 'FontSize', 14);
saveas(gcf, fullfile(input_dir, sprintf('bez_tuning_comparison_%ddB.png', unique_db(db_idx))));
